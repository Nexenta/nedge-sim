...
{
    unsigned t;
    unsigned sum,min,max;
    float avg;
    target_t *tp;
    
    fprintf(f,"%s,0x%lx",tag,now);
    ...
    if (avg)
    	fprintf(f,"\nper-target chunks in-progress: min %d (%.2f * avg) average %3.1f max %d (%.2f * avg)\n",
		min, (float)min/avg, avg, max, (float)max/avg);  
}
