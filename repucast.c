...
//
// Variation of replicast that uses group consensus
// The difference is that the response is multicast to the Negotiating Group
// Each member then calculates the Accept message themselves, and one of them
// unicasts the accept message to the gateway
//
// This eliminates the provisional reservation.
//
// Accept best <n_replicas> as they come in.
// Do unicast transfers to the selected set.
...
typedef enum rg_event_type  { // exends enum event_type
    RG_CHUNK_PUT_READY = TRANSPORT_EVENT_BASE,
    RG_CHUNK_PUT_REQUEST_RECEIVED,
    RG_CHUNK_PUT_RESPONSE_RECEIVED,
    RG_CHUNK_PUT_ACCEPT_RECEIVED,
...
static void handle_rep_chunk_put_response_received (const event_t *e)
{
    ...
    accept_event.event.type = (event_type_t)RG_CHUNK_PUT_ACCEPT_RECEIVED;
    ...
    rendezvous_xfer_event.event.type =
        (event_type_t)RG_RENDEZVOUS_XFER_RECEIVED;
...
static void handle_rep_chunk_put_ready (const event_t *e)
{
    ...
    cprr.event.type = (event_type_t)RG_CHUNK_PUT_REQUEST_RECEIVED;
...

{
    ...
    config.replicast_packet_processing_penalty;
    cpresp.event.type = (event_type_t)RG_CHUNK_PUT_RESPONSE_RECEIVED;
    cpresp.cp = cpr->cp;
